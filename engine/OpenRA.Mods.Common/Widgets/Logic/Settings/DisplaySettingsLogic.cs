...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class DisplaySettingsLogic : ChromeLogic
	{
		...
		static readonly RendererBackend OriginalRendererBackend;
		...
		static DisplaySettingsLogic()
		{
			...
			OriginalRendererBackend = original.Graphics.RendererBackend;
		}
		...
		Func<bool> InitPanel(Widget panel)
		{
			...
			var rendererBackendLabel = new CachedTransform<RendererBackend, string>(p => p.ToString());
			var rendererBackendDropdown = panel.GetOrNull<DropDownButtonWidget>("RENDERER_BACKEND_DROPDOWN");
			if (rendererBackendDropdown != null)
			{
				rendererBackendDropdown.OnMouseDown = _ => ShowRendererBackendDropdown(rendererBackendDropdown, ds);
				rendererBackendDropdown.GetText = () => rendererBackendLabel.Update(ds.RendererBackend);
			}
			...
			glProfileDropdown.IsDisabled = () => disableProfile || ds.RendererBackend == RendererBackend.Vulkan;
			...
			restartDesc.IsVisible = () => ds.Mode != OriginalGraphicsMode || ds.VideoDisplay != OriginalVideoDisplay ||
				ds.GLProfile != OriginalGLProfile || ds.RendererBackend != OriginalRendererBackend ||
			...
			return () =>
			{
				...
				return ds.Mode != OriginalGraphicsMode ||
					...
					ds.RendererBackend != OriginalRendererBackend;
			};
		}
		...
		Action ResetPanel(Widget panel)
		{
			...
			return () =>
			{
				...
				ds.RendererBackend = dds.RendererBackend;
		...
		static void ShowRendererBackendDropdown(DropDownButtonWidget dropdown, GraphicSettings s)
		{
			ScrollItemWidget SetupItem(RendererBackend o, ScrollItemWidget itemTemplate)
			{
				var item = ScrollItemWidget.Setup(itemTemplate,
					() => s.RendererBackend == o,
					() => s.RendererBackend = o);
			...
			var renderers = new[] { RendererBackend.OpenGL, RendererBackend.Vulkan };
			dropdown.ShowDropDown("LABEL_DROPDOWN_TEMPLATE", 500, renderers, SetupItem);
		}
