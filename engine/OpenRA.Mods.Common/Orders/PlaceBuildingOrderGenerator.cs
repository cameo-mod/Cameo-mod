			...
			if (mi.Button == MouseButton.Left)
			{
				...
				if (plugInfo != null)
				{
					...
					if (!AcceptsPlug(topLeft, plugInfo))
					{
						...
						TextNotificationsManager.AddTransientLine(owner, textNotification);
		...
		IEnumerable<IRenderable> IOrderGenerator.RenderAboveShroud(WorldRenderer wr, World world)
		{
			...
			else
			{
				...
				foreach (var t in buildingInfo.Tiles(topLeft))
					footprint.Add(t, MakeCellType(isCloseEnough && world.IsCellBuildable(t, topLeft, actorInfo, buildingInfo)));
			}
