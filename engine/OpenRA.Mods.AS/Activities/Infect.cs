...
{
	sealed class Infect : Enter
	{
		readonly AttackInfect infector;
		readonly AttackInfectInfo info;
		readonly Target target;

		bool jousting;

		public Infect(Actor self, Target target, AttackInfect infector, AttackInfectInfo info, Color? targetLineColor)
		...
		protected override void OnEnterComplete(Actor self, Actor targetActor)
		{
			self.World.AddFrameEndTask(w =>
			{
				...
				infectable.FirepowerMultipliers = self.TraitsImplementing<IFirepowerModifier>()
					.Select(a => a.GetFirepowerModifier(infector.InfectInfo.Name)).ToArray();
