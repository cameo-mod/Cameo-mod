...
namespace OpenRA.Platforms.Default
{
	sealed class Sdl2PlatformWindow : ThreadAffine, IPlatformWindow
	{
		...
		[DllImport("user32.dll")]
		static extern IntPtr MonitorFromPoint(Win32Point pt, uint flags);
		...
		static void EnableWindowsDpiAwareness()
		{
			...
			{
				// Fall back to the older process-wide awareness API below.
			}

			SetProcessDPIAware();
		}

		static float WindowsMonitorScale(IntPtr monitor, float fallback = 1f)
		{
			...
			{
				// GetDpiForMonitor is unavailable on older Windows versions.
			}

			return fallback;
		}

		static float WindowsDisplayScale(int videoDisplay, float fallback = 1f)
		{
			if (SDL.SDL_GetDisplayBounds(videoDisplay, out var bounds) != 0)
				return fallback;

			var monitor = MonitorFromPoint(new Win32Point(bounds.x + bounds.w / 2, bounds.y + bounds.h / 2), MonitorDefaultToNearest);
			return WindowsMonitorScale(monitor, fallback);
		}

		static float WindowsWindowScale(IntPtr window, float fallback = 1f)
		{
			var info = default(SDL.SDL_SysWMinfo);
			SDL.SDL_VERSION(out info.version);
			if (SDL.SDL_GetWindowWMInfo(window, ref info) == SDL.SDL_bool.SDL_TRUE && info.info.win.window != IntPtr.Zero)
				return WindowsMonitorScale(MonitorFromWindow(info.info.win.window, MonitorDefaultToNearest), fallback);
		...
		public Sdl2PlatformWindow(Size requestEffectiveWindowSize, WindowMode windowMode,
			...
			: this(requestEffectiveWindowSize, windowMode, scaleModifier, vertexBatchSize, indexBatchSize, videoDisplay, requestProfile, RendererBackend.OpenGL) { }

		public Sdl2PlatformWindow(Size requestEffectiveWindowSize, WindowMode windowMode,
			float scaleModifier, int vertexBatchSize, int indexBatchSize, int videoDisplay, GLProfile requestProfile, RendererBackend rendererBackend)
		{
			...
			lock (syncObject)
			{
				...
				if (rendererBackend == RendererBackend.OpenGL)
				{
					...
					Console.WriteLine("Using SDL 2 with Vulkan renderer");
				...
				if (Platform.CurrentPlatform == PlatformType.Windows)
				{
					...
					if (scaleVariable == null || !float.TryParse(scaleVariable, NumberStyles.Float, NumberFormatInfo.InvariantInfo, out windowScale) || windowScale <= 0)
						windowScale = WindowsDisplayScale(videoDisplay);
				}
				...
				var windowFlags = SDL.SDL_WindowFlags.SDL_WINDOW_ALLOW_HIGHDPI;
				if (rendererBackend == RendererBackend.OpenGL)
					windowFlags |= SDL.SDL_WindowFlags.SDL_WINDOW_OPENGL;
				else if (rendererBackend == RendererBackend.Vulkan)
				...
				if (Platform.CurrentPlatform == PlatformType.Windows)
				{
					var actualWindowScale = WindowsWindowScale(Window, windowScale);
					if (Math.Abs(actualWindowScale - windowScale) > 0.001f)
					{
						windowScale = actualWindowScale;
						if (requestEffectiveWindowSize.Width != 0 || requestEffectiveWindowSize.Height != 0)
						{
							surfaceSize = new Size((int)(requestEffectiveWindowSize.Width * windowScale), (int)(requestEffectiveWindowSize.Height * windowScale));
							SDL.SDL_SetWindowSize(Window, surfaceSize.Width, surfaceSize.Height);
						}
					}
				}
				...
				if (windowMode == WindowMode.Fullscreen)
				{
					...
					SDL.SDL_SetWindowPosition(Window, SDL.SDL_WINDOWPOS_CENTERED_DISPLAY(videoDisplay), SDL.SDL_WINDOWPOS_CENTERED_DISPLAY(videoDisplay));
			...
			if (rendererBackend == RendererBackend.Vulkan)
			{
				Context = new VulkanGraphicsContext(this, Game.Settings.Graphics.VSync);
			}
			else if (Platform.CurrentPlatform == PlatformType.Windows && windowMode == WindowMode.Windowed)
			{
