	...
	public sealed class Renderer : IDisposable
	{
		...
		public Renderer(IPlatform platform, GraphicSettings graphicSettings)
		{
			...
			Window = platform.CreateWindow(new Size(resolution.Width, resolution.Height),
				...
				graphicSettings.VideoDisplay, graphicSettings.GLProfile, graphicSettings.RendererBackend);
		...
		void BeginFrame()
		{
			...
			if (screenSprite == null || surfaceSize.Width != Math.Abs(screenSprite.Bounds.Width) || surfaceSize.Height != Math.Abs(screenSprite.Bounds.Height))
			{
				var screenSheet = new Sheet(SheetType.BGRA, screenBuffer.Texture);

				var screenBounds = Context.RequiresRenderTargetYFlip
					// Flip sprite in Y to match OpenGL's bottom-left origin.
					? Rectangle.FromLTRB(0, surfaceSize.Height, surfaceSize.Width, 0)
					// Vulkan already presents render targets with the same top-left
					// orientation as OpenRA's UI coordinate system.
		...
		public void BeginUI()
		{
			if (renderType == RenderType.World)
			{
				...
				var bufferScale = new float3(
					(int)(Math.Abs(screenSprite.Bounds.Width) / scale) / worldSprite.Size.X,
					(int)(Math.Abs(screenSprite.Bounds.Height) / scale) / worldSprite.Size.Y,
					1f);
		...
		public void EndFrame(IInputHandler inputHandler)
		{
			...
			if (Context.RequiresRenderTargetYFlip)
				...
				RgbaSpriteRenderer.DrawSprite(screenSprite, float3.Zero,
					new float3(lastBufferSize.Width / screenSprite.Size.X, lastBufferSize.Height / screenSprite.Size.Y, 1f));
			Flush();
		...
		public void SaveScreenshot(string path)
		{
			...
			var destWidth = Math.Abs(screenSprite.Bounds.Width);
			var destHeight = Math.Abs(screenSprite.Bounds.Height);
