...
namespace OpenRA
{
	...
	public interface IPlatform
	{
		IPlatformWindow CreateWindow(
			Size size, WindowMode windowMode, float scaleModifier, int vertexBatchSize, int indexBatchSize, int videoDisplay,
			GLProfile profile, RendererBackend rendererBackend);
	...
	public interface IGraphicsContext : IDisposable
	{
		...
		/// <summary>
		/// OpenGL render targets use a lower-left framebuffer origin, so the final
		/// render-to-window compositor must flip them vertically. Backends with a
		/// top-left render target origin, such as the Vulkan path, must not apply
		/// this extra OpenGL compatibility flip.
		/// </summary>
		bool RequiresRenderTargetYFlip { get; }
	}
