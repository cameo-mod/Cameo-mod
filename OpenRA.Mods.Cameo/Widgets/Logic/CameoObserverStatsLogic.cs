...
namespace OpenRA.Mods.Cameo.Widgets.Logic
{
	...
	public class CameoObserverStatsLogic : ChromeLogic
	{
		...
		CameoObserverStatsPanel activePanel;

		[ObjectCreator.UseCtor]
		public CameoObserverStatsLogic(World world, ModData modData, WorldRenderer worldRenderer, Widget widget, Dictionary<string, MiniYaml> logicArgs)
		{
			this.world = world;
			this.worldRenderer = worldRenderer;

			MiniYaml yaml;
			var keyNames = Enum.GetNames(typeof(CameoObserverStatsPanel));
			...
			var statsDropDownOptions = new StatsDropDownOption[]
			{
				new()
				{
					...
					OnClick = () =>
					{
						...
						ClearStats();
						activePanel = CameoObserverStatsPanel.None;
					}
				},
				CreateStatsOption(Minimal, CameoObserverStatsPanel.Minimal, minimalPlayerTemplate, () => DisplayStats(MinimalStats)),
				CreateStatsOption(Basic, CameoObserverStatsPanel.Basic, basicPlayerTemplate, () => DisplayStats(BasicStats)),
				CreateStatsOption(Economy, CameoObserverStatsPanel.Economy, economyPlayerTemplate, () => DisplayStats(EconomyStats)),
				CreateStatsOption(Production, CameoObserverStatsPanel.Production, productionPlayerTemplate, () => DisplayStats(ProductionStats)),
				CreateStatsOption(SupportPowers, CameoObserverStatsPanel.SupportPowers, supportPowersPlayerTemplate, () => DisplayStats(SupportPowerStats)),
				CreateStatsOption(Combat, CameoObserverStatsPanel.Combat, combatPlayerTemplate, () => DisplayStats(CombatStats)),
		...
		{
			promotionsHeaders.Visible = true;
			var template = SetupPlayerScrollItemWidget(promotionsPlayerTemplate, player);
			...
			var playerPromotions = player.PlayerActor.TraitOrDefault<PlayerPromotions>();
			if (playerPromotions != null)
			{
				var promotionRank = template.Get<LabelWidget>("PROMOTION_RANK");
				promotionRank.GetText = () => playerPromotions.currentRank;

				var promotionProgress = template.Get<LabelWidget>("PROMOTION_PROGRESS");
				var progressText = new CachedTransform<(int playerProgress, int progressRequired), string>(p => p.playerProgress + (playerPromotions.IsMaxLevel ? null : ("/" + p.progressRequired)));
				promotionProgress.GetText = () => progressText.Update((playerPromotions.Experience, playerPromotions.nextLevelXpRequired));

				var promotionPoints = template.Get<LabelWidget>("PROMOTION_POINTS");
				promotionPoints.GetText = () => playerPromotions.Points.ToString(NumberFormatInfo.CurrentInfo);
			}

			template.Get<ObserverPromotionsIconsWidget>("PROMOTIONS_ICONS").GetPlayer = () => player;
			template.IgnoreChildMouseOver = false;
