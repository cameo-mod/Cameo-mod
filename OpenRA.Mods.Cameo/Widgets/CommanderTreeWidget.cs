...
namespace OpenRA.Mods.Cameo.Widgets
{
	public class CommanderTreeWidget : Widget
	{
		...
		// Stage B (DecoupledRendering): queue head cached in Tick under the world read lock; Draw must not
		...
		public override bool HandleMouseInput(MouseInput mi)
		{
			...
			// Stage B (DecoupledRendering): the click handlers read live queue state (CanQueue) - wait out any
		...
		// Stage B (DecoupledRendering): set once the first UpdateNodes has run, so we only BLOCK for the world
		// lock on the initial populate (see Tick).
		bool hasPopulated;

		public override void Tick()
		{
			// Stage B (DecoupledRendering): EnsureQueue/UpdateNodes enumerate the live queue state (AllQueued,
		...
		void UpdateNodes()
		{
			...
			// Stage B (DecoupledRendering): Draw must not call into the live queue. IsProducing and RemainingTimeActual
