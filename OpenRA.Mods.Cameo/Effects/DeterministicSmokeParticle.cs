...
{
	sealed class DeterministicSmokeParticle : IEffect
	{
		...
		public DeterministicSmokeParticle(Actor invoker, ISmokeParticleInfo smoke, WPos pos, int facing = -1, bool visibleThroughFog = false)
		{
			...
			this.facing = facing > -1 ? facing : world.SharedRandom.Next(256);
			turnRate = smoke.TurnRate;

			anim = new Animation(world, smoke.Image, () => WAngle.FromFacing(this.facing));
			...
			lifetime = smoke.Duration.Length == 2 ? world.SharedRandom.Next(smoke.Duration[0], smoke.Duration[1]) : smoke.Duration[0];
		...
		public void Tick(World world)
		{
			...
			var forward = speed.Length == 2 ? world.SharedRandom.Next(speed[0].Length, speed[1].Length) : speed[0].Length;
			var height = gravity.Length == 2 ? world.SharedRandom.Next(gravity[0].Length, gravity[1].Length) : gravity[0].Length;
