...
namespace OpenRA.Mods.Cameo.Warheads
{
	...
	public class AreaDamagePercentageWarhead : AreaDamageWarhead
	{
		...
		protected override void ValidateFields()
		{
			...
			// The inherited folded half derives a percentage from Damage, but Damage
			// on this subclass is already a percentage. Combining both is an accidental
			// double percentage hit with no stable flat-damage coefficient to price.
			if (PercentageScale > 0)
				throw new YamlException("AreaDamagePercentage cannot also set PercentageScale; "
					+ "use a flat AreaDamage warhead for folded percentage damage.");
		}
