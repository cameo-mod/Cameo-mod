...
- **Removal markers (`-Warhead@X:` or `-ActorX:`) crash if the removed key no longer exists in the resolved chain.** `8Inch` had `-Warhead@Effect2:` inherited from the old `^Grenade`/`^HeavyBomb` stack. After repointing to `^Effect_Demolition_Light`, `Effect2` was gone and the game threw "There are no elements with key `Warhead@Effect2` to remove". Any conversion must strip stale removal markers before boot-gating. For mechanical scripts, pre-emptively removing any `-Warhead@` line (plus its child lines) from converted blocks is safe because the new `^Effect_*` templates do not use removal markers.
