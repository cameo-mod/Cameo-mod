...
> ⚠ **This law is currently VIOLATED and needs a maintainer ruling (2026-08-11).**
> `c9a09dc91` added five DERIVED fields to every ledger row — `effective_damage`,
> `effective_base_total`, `effective_footprint_cells2`, `effective_avg_reliability`,
> `effective_sigma` (spec: [`EFFECTIVE_DAMAGE.md`](EFFECTIVE_DAMAGE.md)).
>
> The concrete cost: a derived field changes when the **formula** changes, with no yaml
> change at all. Correcting the metric's scatter model on 2026-08-11 rewrote **4 136
> ledger lines while `mods/` was untouched** — noise inside the very artifact whose job
> is to prove yaml ↔ ledger equality (`audit_balance_drift`).
>
> **Recommendation:** keep the raw ledger pure and emit derived metrics to a separate,
> clearly-generated artifact (`docs/balance/derived/*.json`), regenerated by the same
> `extract_stats.py` run. Then a ledger diff always means "a real stat moved", and a
> derived diff always means "the model changed" — two different review questions that
> are currently mixed into one file. Until this is decided the fields stay where they
> are; do not build anything that depends on their location.
