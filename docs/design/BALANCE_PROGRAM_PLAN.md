...
| **W24** | Collapse every weapon to ONE damage warhead (3-way split, damage half) — 61% of weapons carry 2+, worst case 15 | 🔵 second cluster collapsed (`SporemawShoot` + `wc2demolitionsquadExplode` + `wc2mageFireballVisible`/`Explosion` + `wc2ogremageRunes_Hit`) to `^Warhead_CannonAP_Light`; corrected `wc2cannontowerFire` → `CannonHE_Heavy` and `wc2dragonFireVisible` → `Flame_Heavy`; Protoss `^Corrodible` moved to `^LargeProtoss`; broadcast baseline lowered to 977 | Claude | — |
