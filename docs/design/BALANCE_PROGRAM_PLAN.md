...
| **W24** | Collapse every weapon to ONE damage warhead (3-way split, damage half) — 61% of weapons carry 2+, worst case 15 | 🔵 fifth cluster collapsed (`TSTibBazooka` + `TSChemBazooka`) to `^Warhead_MissileAP_Light`; preserved `Corrosion` physical state with scaled `PhysicalStateScale: 20`; kept `TSChemBazooka` smoke cloud; broadcast baseline remains 970 | Claude | — |
