...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — collapsed the `TS70mmTurChem`/`TSScoopDualTurChem` pair, the four light-chemical/AP-cannon weapons (`TSHighVelocityChem`, `TSHighVelocity2Chem`, `TSHighVelocityTurChem`, `CabalDissolverSpray`) onto CannonChem, and the exact medium flame+chemical pair (`PlasmaFlamer`, `MutFlamerChem`) onto Plasma. Flat and percentage totals, projectile operation, sounds, custom conditions/clouds, effects, smudges, shield behavior, and concrete damage are preserved; standard family profiles are accepted classification consequences. `review_batch_diff.py` now gates percentage totals as well as flat damage. Survey 294 → 286 and W24 broadcast debt 936 → 932. Static audit-gated; in-game review deferred by maintainer request. |
