...
| **W24** | Collapse every weapon to ONE damage warhead (3-way split, damage half) — 61% of weapons carry 2+, worst case 15 | 🔵 third cluster collapsed (`FutureJavelinRockets` + `FutureJavelinRockets_elite`/`Deployed`/`Deployed_elite` + `Future_MultiMissile_Javelin`) to `^Warhead_MissileAP_Light`; previous CannonHE/Flame/Protoss corrections remain; broadcast baseline lowered to 972 | Claude | — |
