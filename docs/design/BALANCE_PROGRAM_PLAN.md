...
| **W24** | Collapse every weapon to ONE damage warhead (3-way split, damage half) — 61% of weapons carry 2+, worst case 15 | 🔵 fourth cluster collapsed (`Dragon`) to `^Warhead_MissileAA_Heavy` with `^Projectile_Flak_Heavy`/`^Effect_Flak_Heavy`; previous clusters and corrections remain; broadcast baseline lowered to 970 | Claude | — |
