...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The new safe cleanup consolidates nine missile roots, covering fourteen resolved definitions, onto their standard missile families while keeping independently rounded percentage and presentation compatibility behavior. Three roots split the standard profile only to preserve the portion of their old flat damage that could not hit walls. Two Waveforce armored-car variants also retire their last old anti-air flat hit while retaining its independently rounded percentage result explicitly. `review_batch_diff.py` now tests all 155 active/design health values and gates complete top-level operation, projectiles, non-damage warheads, and damage-target exclusions as well as flat and percentage damage. The corrected survey counts only winning active definitions and now reports 274 concrete legacy-family weapons, including 267 mixed weapons in 203 groups; broadcast debt is 923. Pricing and the runtime fix remain separate. |
