...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots, the MiG, Naxis quad-cannon/flak, RA2 SCUD, and Steel Mako families, four isolated weapons, the bulk shotgun/sniper pass, thirteen named laser roots covering 27 resolved definitions, the remaining GDI/commando/Dragunov sniper roots, and a percentage-safe 13-root chemical/flame cohort covering 15 definitions. The newest cohort keeps every legacy percentage application and its complete armor/shape/target/statistics profile separate while moving only flat damage onto standard chemical cannon, chemical, flame, and chemical missile families. `review_batch_diff.py` tests all 155 active/design health values and now fingerprints complete percentage-warhead profiles in addition to relationship-, statistics-, and physical-state-specific behavior, top-level operation, projectiles, non-damage warheads, damage-target exclusions, and percentage damage. On the corrected active survey basis the debt has fallen from 274 to 237 concrete roots: 234 mixed weapons in 186 groups and three isolated roots. The conservative classifier now reports 16 corroborated, 26 legacy-only, and 195 human-decision roots. Broadcast debt is ratcheted to 878. Pricing and the runtime fix remain separate. |
