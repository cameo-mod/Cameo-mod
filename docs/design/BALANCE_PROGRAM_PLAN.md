...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots, the MiG, Naxis quad-cannon/flak, RA2 SCUD, and Steel Mako families, four isolated weapons, and the new bulk shotgun/sniper pass: four shotgun roots (seven resolved definitions) use medium CannonHE and four sniper roots (eleven resolved definitions) use the infantry-favoured heavy Bullet profile. Sniper damage uses a one-world-unit impact footprint, removing practical splash while retaining reduced vehicle damage. Separate compatibility slices preserve hit counts, per-hit rounding, target and relationship splits, score accounting, exclusions, physical-state bindings, and independently rounded percentage paths. `review_batch_diff.py` tests all 155 active/design health values and gates relationship-, statistics-, and physical-state-specific behavior in addition to complete top-level operation, projectiles, non-damage warheads, damage-target exclusions, and percentage damage. The survey now reads the active D2K, StarCraft, and Outpost 2 central files and correctly recognizes `^Template` block boundaries. On that corrected basis this batch reduces the debt from 274 to 266 concrete roots: 263 mixed weapons in 201 groups and three isolated roots. The conservative classification report keeps 205 roots in human review while prioritizing 31 where name and legacy evidence agree and 30 with a legacy-only suggestion; its JSON preserves full family-and-tier identities and records flat/percentage hit inventories, physical-state bindings, descendant closure, and descendant old-key overrides. Broadcast debt remains guarded. Pricing and the runtime fix remain separate. |
