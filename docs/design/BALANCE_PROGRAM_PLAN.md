...
| **W24** | Collapse every weapon to ONE damage warhead (3-way split, damage half) — 61% of weapons carry 2+, worst case 15 | ✅ sixth cluster (`TSChemJuggerboat90mm`/`TSChemVanMissile`/`TSChemMLRSMissile`) collapsed to `^Warhead_Concussion_Medium`; maintainer review corrected family misassignments: `TSTibBazooka`/`TSChemBazooka` → `^Warhead_ChemMissile_Light`, `Dragon` → `^Projectile_Missile_Heavy`/`^Effect_MissileHE_Heavy`, `FutureJavelinRockets` → `MissileAP_Medium`, `Future_MultiMissile_Javelin` → `MissileAP_Heavy`, `Future_MultiMissile_Frag` → `Concussion_Heavy`, `SporemawShoot` → `Concussion_Medium`+grenade, `wc2demolitionsquadExplode` → `Demolition_Heavy`, `wc2mageFireballVisible` → `Flame_Heavy`, `wc2dragonFireVisible` effect cleanup; `find_empty_warhead` 0, `audit_warhead_split` baseline 969, `audit_duplicate_keys` D1 baseline 89, boot-gated; `ff0ffeb90` | Claude | — |
