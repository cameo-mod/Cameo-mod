...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — first pair collapsed `TS70mmTurChem` (3 × 4000 → `^Warhead_CannonChem_Light` 12000) and `TSScoopDualTurChem` (3 × 16000 → `^Warhead_CannonChem_Medium` 48000); next group collapsed `TSHighVelocityChem`, `TSHighVelocity2Chem`, `TSHighVelocityTurChem`, and `CabalDissolverSpray` from paired light-chemical/AP-cannon mains onto `^Warhead_CannonChem_Light` at 45000/60000/72000/4000. Flat and percentage totals, projectile operation, sounds, custom conditions/clouds, effects, smudges, shield behavior, and concrete damage are preserved; standard CannonChem family profiles are accepted classification consequences. `review_batch_diff.py` now gates percentage totals as well as flat damage. Survey 294 → 288 and W24 broadcast debt 936 → 933. Static audit-gated; in-game review deferred by maintainer request. |
