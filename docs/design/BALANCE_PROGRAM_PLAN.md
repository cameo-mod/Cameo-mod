...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — collapsed the chemical turret pair; four light-chemical/AP-cannon weapons onto CannonChem; the exact medium flame+chemical pair onto Plasma; two redundant medium+heavy Flame stacks onto Flame Heavy; and three redundant light+medium+heavy Chemical stacks onto Chemical Heavy. Flat and percentage totals, projectile operation, sounds, custom conditions/clouds, effects, smudges, shield behavior, and concrete damage are preserved; standard family profiles are accepted classification consequences. `review_batch_diff.py` now gates percentage totals as well as flat damage. Survey 294 → 281 and W24 broadcast debt 936 → 927. Static audit-gated; in-game review deferred by maintainer request. |
