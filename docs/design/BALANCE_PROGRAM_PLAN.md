...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots plus the active MiG missile family, covering ten more ground-attack, anti-air, radioactive, incendiary, tesla, and elite definitions. Compatibility-only standard-profile slices preserve wall and Ground/Ship targeting differences without entering the shared pricing model, while independently rounded percentage and presentation behavior remains active. Two Waveforce armored-car variants also retire their last old anti-air flat hit while retaining its percentage result explicitly. `review_batch_diff.py` tests all 155 active/design health values and gates complete top-level operation, projectiles, non-damage warheads, and damage-target exclusions as well as flat and percentage damage. The corrected survey now reports 273 concrete legacy-family weapons, including 266 mixed weapons in 202 groups; broadcast debt remains guarded. Pricing and the runtime fix remain separate. |
