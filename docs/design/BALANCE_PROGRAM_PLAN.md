...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots, the MiG, Naxis quad-cannon/flak, RA2 SCUD, and Steel Mako families, plus four final isolated weapons: the FutureTech cryocopter rocket, anti-tank mine, Waveforce chain gun, and Tiberian Sun laser 90mm family. Compatibility-only standard-profile slices preserve target and relationship splits, score accounting, physical-state bindings, and separately rounded percentage paths without entering the shared pricing model. `review_batch_diff.py` tests all 155 active/design health values and gates relationship-, statistics-, and physical-state-specific behavior in addition to complete top-level operation, projectiles, non-damage warheads, damage-target exclusions, and percentage damage. The refreshed survey reports 266 concrete legacy-family weapons: 265 mixed weapons in 201 groups plus the isolated `RA2CRM60H`, whose heavy-cannon versus medium-bullet classification and passenger-only damage require an explicit maintainer decision. Broadcast debt remains guarded. Pricing and the runtime fix remain separate. |
