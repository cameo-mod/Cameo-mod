...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots, the MiG, Naxis quad-cannon/flak, RA2 SCUD, and Steel Mako families, four isolated weapons, the bulk shotgun/sniper pass, six named laser roots covering eight resolved definitions, and the remaining GDI/commando/Dragunov sniper roots. The latest sniper follow-up makes every damage path direct-hit only; GDI and Commando use the infantry-favoured heavy Bullet profile, while Dragunov retains a heavy CannonAP anti-materiel profile but cannot one-shot or win a stationary duel against a baseline Mammoth Tank. `review_batch_diff.py` tests all 155 active/design health values and gates relationship-, statistics-, and physical-state-specific behavior in addition to complete top-level operation, projectiles, non-damage warheads, damage-target exclusions, and percentage damage. The survey reads the active D2K, StarCraft, and Outpost 2 central files and correctly recognizes `^Template` block boundaries. On that corrected basis the debt has fallen from 274 to 257 concrete roots: 254 mixed weapons in 193 groups and three isolated roots. The conservative classification report keeps 202 roots in human review while prioritizing 25 where name and legacy evidence agree and 30 with a legacy-only suggestion; its JSON preserves full family-and-tier identities and records flat/percentage hit inventories, physical-state bindings, descendant closure, and descendant old-key overrides. Broadcast debt remains guarded. Pricing and the runtime fix remain separate. |
