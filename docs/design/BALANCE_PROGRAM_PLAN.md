...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — first pair collapsed `TS70mmTurChem` (3 × 4000 → `^Warhead_CannonChem_Light` 12000) and `TSScoopDualTurChem` (3 × 16000 → `^Warhead_CannonChem_Medium` 48000). Projectile behavior, sounds, impact effects, smudges, clouds, water/air effects, and concrete damage are preserved locally; standard CannonChem armour/blast profiles are accepted classification consequences. Survey 294 → 292 and W24 broadcast ratchet 936 → 934. Static audit-gated; in-game review deferred by maintainer request. |
