...
| A6 | Continue the burn-down | 🔵 IN PROGRESS — retrospective review restored earlier weapon blocks whose grouped percentage hits could not reproduce the parked runtime's per-warhead rounding and overflow behavior at every active health value. The safe cleanup now includes nine earlier missile roots, the active MiG family, the twelve-definition Naxis quad-cannon/flak family, and the seven-definition RA2 SCUD family. Compatibility-only standard-profile slices preserve wall, Ground/Ship, Ground/Water splash, allied-damage, and score-accounting differences without entering the shared pricing model, while independently rounded percentage and presentation behavior remains active. Two Waveforce armored-car variants also retire their last old anti-air flat hit while retaining its percentage result explicitly. `review_batch_diff.py` tests all 155 active/design health values and gates relationship- and statistics-specific flat damage in addition to complete top-level operation, projectiles, non-damage warheads, damage-target exclusions, and percentage damage. The corrected survey now reports 271 concrete legacy-family weapons, including 265 mixed weapons in 201 groups; broadcast debt remains guarded. Pricing and the runtime fix remain separate. |
