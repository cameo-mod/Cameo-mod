...
| 5 | Implement the bell in `gen_weapon_template`, then `AreaDamageWarhead` | ✅ `gen_weapon_template` bell implemented (OFF by default); ✅ `AreaDamageWarhead` C# transform wired (inert at Heaviness 0); spread scale still pending a ruling |
