...
## 1b. Weapon naming (maintainer 2026-08-29) — binding

```
weapon id := <actor_id>_<family>[_<qualifier>][_<variant>][_<structural>]
shared    := shared_<namegroup>
```

> *"The naming should be simple: the actor name plus the weapon type ... take into
...
Generated by **`tools/rename/gen_weapon_names.py`**, which derives every token from
the resolved tree. Nothing here is chosen by hand.

| token | source | example |
|---|---|---|
| `actor_id` | the single actor whose `Armament` fires it | `td_gdi_mammothtank` |
| `family` | base of `Inherits@wh: ^Warhead_<Family>_<Level>`, CamelCase-split, lowercased | `CannonHE` → `cannon` |
| `qualifier` | the rest of that family — **only** to break a tie between two weapons of the same base on one actor | `_he` vs `_ap` |
| `variant` | the upgrade that swaps the weapon in | `_hypervelocity` |
| `structural` | DESIGN §1 suffixes carried over verbatim | `_aa`, `_emp`, `_elite` |

Worked example, the maintainer's own:

```
120mm       -> td_gdi_battletank_cannon_he
120mmHV     -> td_gdi_battletank_cannon_he_hypervelocity
...
* ⛔ **A NEGATED condition means the BASE weapon.** An upgrade pair is wired `!x` on
  the base and `x` on the replacement. Reading the token without the `!` names both
  after the upgrade — which made the plain Battle Tank cannon
...
The `family` token comes from the 3-way split's `^Warhead_` inherit, and only
**49.2%** of live weapons carry one (806 of 1637; 307 are still on legacy templates
like `^Chaingun`, 524 on none). Renaming now would name half the roster correctly,
guess at the rest, and be invalidated the moment W27 rewrites the very inheritance
the names are read from. `gen_weapon_names.py --write` **refuses** below
`--min-coverage` (default 95%) for exactly this reason.

**Order: W27 weapon structure → regenerate → review collisions → `safe_rename.py`
→ boot-gate.** `tools/rename/rename_map_weapons.yaml` (1560 entries, generator
