# Peer `Versus` armor vocabularies — the input to armor-aware effective DPS
...
**76 distinct armor tags across 13 mods.** Only **five** are shared by six or more: `None`, `Light`, `Heavy`, `Wood`, `Concrete` — the Westwood core. Everything else is source-specific. Three findings make a guessed taxonomy indefensible:
...
* **OpenRA Dune II declares NO Versus at all** — it cannot participate in armor-aware DPS.
...
⚠ `Versus` is a NODE WITH AN EMPTY VALUE whose CHILDREN are the armor rows. A probe reading `node.get("Versus")` gets the empty value and concludes the mod has none — which is how this measurement first came back as "0 peers expose Versus" for all 13.
...
| OpenE2140 | 4 | — | `aircraft`, `building`, `infantry`, `vehicle` |
...
| OpenRA Tiberian Dawn | 5 | `Concrete`, `Heavy`, `Light`, `None`, `Wood` | — |
| OpenRA Tiberian Sun | 5 | `Concrete`, `Heavy`, `Light`, `None`, `Wood` | — |
...
Part 1 of the weapon layer — range, damage, burst, reload and raw sustained DPS — needs none of this and is already measured. Part 2, **armor-aware effective DPS**, needs a mapping table carrying `source_tag`, `normalized_class`, `confidence` and `reason` per source. The Westwood core maps at high confidence; everything else needs a judgement, and a few sources cannot participate at all.
