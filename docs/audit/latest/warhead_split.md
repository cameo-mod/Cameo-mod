...
## FAIL 1 — broadcast fingerprint / every MAIN identical (936 vs baseline 936)

_at or below baseline_ — pre-existing **W24** debt (936 weapons), not a regression. The ratchet catches new broadcasts without blocking every commit on the existing pile. **Lower `BROADCAST_BASELINE` as W24 collapses weapons; never raise it.**
...
## Review — high uniform stacks (informational, 236)
