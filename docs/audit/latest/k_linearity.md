...
_clean_ — modeled 2474 folded and 2292 standalone applications.
...
`k == k_flat + (pct_absolute + folded_rounding) / damage_total`. The standalone term is a floor; folded rounding is only the current quantisation residual.
...
## L4 — folded basis-point rounding residual

41 weapon(s) have a non-zero current folded rounding residual.
