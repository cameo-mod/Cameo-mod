...
- Formula-only price effect of the current hidden stat buff (excluding its damage-taken term): **16.5%**.
...
The formula-only price effect of the current hidden stat buff is about 16.5% after removing the buff. Across the current four-faction promotion sample, 1500-per-tier is closer to that scale than 2000-per-tier while still giving later promotion tiers more room; the rational curve can leave some early units on its C <= B plateau, so a virtual cost is not a guaranteed nonzero discount.
...
- The damage-taken part of `^PromotionUnitBuff` is not represented in the current price formula comparison; survivability must be reviewed separately.
