...
- Unique strict weapons: **28** (raw R1-R4 rows: **33**; R3/R4 duplicates are merged).
- Custom selectors: **27**; explicit policy questions: **3**.

| decision lane | count | meaning |
|---|---:|---|
| DUAL_DOMAIN_ABSOLUTE_RULE | 5 | Both-domain route; MissileHE-against-Air also violates the explicit never rule. Review the complete consumer closure before choosing AP. |
...
| critical | `ra1_allies_rapierjumpjet_missile_AA` | R3, R4 | both | MissileHE | MissileAP | 1 | `mods/cameo/ContentPacks/RedAlert/Allies/yaml/weapons.yaml:360` |
...
Source receipt: `docs/audit/latest/missile_role_triage_20260911.json`.
