...
| **Derived armour types** (cyborg, anti-air, naval; Heroic /200) | [`DESIGN.md`](DESIGN.md) §12.0l — the rulings, the 12 parent pairs, membership, order of work | `tools/balance/gen_weapon_template.py` (`GEO_DERIVED`, `derive_rows`) · `tools/balance/derive_versus_columns.py` (writes the rows into every NON-generated table; idempotent; each lane runs it on its own files) · `tools/audit/audit_derived_armor_columns.py` (lower-only ratchet; must be 0 before any unit wears a derived type) |
...
| **Level retirement / family bases (§12.0j)** | [`DESIGN.md`](DESIGN.md) §12.0j (the 2026-09-26 "retire now, reshape later" amendment and its IMPLEMENTED note) and §12.0c (Shield carry-over) | `gen_weapon_template.py`: `family(..., base=True)` + `family_calls()` + `family_bases()` emit all 52 `^Warhead_<Family>` bases; `splice_templates.py --all` places each after its family; guard `tools/tests/test_family_bases.py`. Re-pointing the weapons is the NEXT step, not done yet |
