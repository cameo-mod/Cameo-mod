...
| **Weapon structure (W24 / W23 / A5)** | [`design/BALANCE_PROGRAM_PLAN.md`](design/BALANCE_PROGRAM_PLAN.md) §0a **order of operations** and §1b **W24 diagnosis**; [`design/WEAPON_3WAY_SPLIT.md`](design/WEAPON_3WAY_SPLIT.md) | ⛔ **`tools/audit/intentional_composites.py` FIRST** — multi-main weapons already REVIEWED and deliberately kept; collapsing one is not your call (`--snapshot` to read it) · `tools/audit/audit_warhead_split.py` · `tools/audit/audit_three_way_split.py` · `tools/audit/audit_unconverted_templates.py` · `tools/audit/review_resolve_diff.py` · `tools/audit/find_empty_warhead.py` |
