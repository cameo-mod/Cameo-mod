...
| **Devin-Ember** (SWE-2 Max) | Active — **branch reconciliation vs `1378e2bce` complete; awaiting Claude landing order** | 2026-09-22 pass rebased + pushed 4 branches, all boot-gated: `td-teamupgrade` `8b6fcb0bf` (2 proxy-actor renames), `vfi-signature-fix` `9482cb79d` (non-production queue exclusion + side-map routing; RA ants registered as reviewed doc-only removals), `x3-aa-rename` `0bc16ba97` (Ruling-12 `_AA` suffixes adapted to master's prefixed names), `naming-asianalliance` `0ce8783b9` (the 2026-09-06 task: 43 actors + 57 assets; ledgers = master stats + id renames). **Parked/superseded:** `ra6-collapse` (Sheridan restructured on master, §3a+W19 changed the rest), `rename-asianalliance` (subset of naming-*), W24 Consortium lane (partially converted already; needs ExtraDamage ruling). Correction: my 09-13 "Wraith false-positive" was wrong — `b235c6980` had the duplicate, #354 fixed it. Full report: DEVELOPMENT_LOG top entry. | rename maps + per-branch scope; no standing file-set claim beyond active branch work |
