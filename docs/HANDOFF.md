...
| **Devin-Aurora** (SWE-1.7 Max / GLM-5.2 High) | Active — **D2k Atreides buildout** (maintainer order 2026-09-05) | D2k Phase 0/1/2/3 coordinator. Committed `cda4c54ec`, `f46e61326`, `c16457655`, `95261becb`, `9cdfa40dd` (ordos_laserturret W23). W24 safe pool EXHAUSTED. **NOW: building Atreides unique weapons from legacy d2k.yaml + setting Selectable: true. Then: HydraSpit/BulletChem rework + cabal_avatar patch.** | `mods/cameo/ContentPacks/D2k/Atreides/`, `mods/cameo/ContentPacks/D2k/Ordos/yaml/weapons.yaml`, `mods/cameo/ContentPacks/D2k/Shared/yaml/weapons.yaml`, `mods/cameo/bits/d2k/` |
...
| **Devin-Nova** (Devin CLI, SWE-1.7 Max) | Active — coordinator/verifier lane | Committed `7557c983d` (AreaDamageWarhead C# NRE fix + GDI stale removals). Tree-wide orphaned-removal sweep (committed by Aurora as `c16457655`). **ORDER: continue verifier lane with Ember. Monitor for boot-blockers. Help with audit suite refresh.** | `OpenRA.Mods.Cameo/Warheads/AreaDamageWarhead.cs`, tree-wide stale-removal sweep |
| **Claude** (Anthropic, SWE-1.7 Max) | Active — **rebase your branches** (Aurora order 2026-09-05) | Your patches (`01_bulletchem_hydraspit.patch`, `02_cabal_avatar_dreadnought.patch`) no longer apply cleanly — tree has moved 95 commits. **ORDER: 1. Aurora is reworking HydraSpit locally. 2. Rebase `claude/*` branches against current `weapon_structure_and_warhead_fold`. 3. Provide missing data sources (DTA, Rise of the East, Emperor: Battle for Dune). 4. Do NOT merge wholesale — extract specific files only.** | `claude/*` branches (155 commits diverged) |
