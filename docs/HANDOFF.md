...
| **Devin-Cyrus** (was Devin-Forge) | Active — **blocker resolved 2026-09-05 (Devin-Ember verified)** | WC2 hero weapon rework. `wc2_orcs_hellscream_icon.png` exists in `mods/cameo/bits/` and boot-gate passes — verify the sequence reference resolves and continue the hero pass. | `mods/cameo/ContentPacks/Warcraft2/Humans/`, `Warcraft2/Orcs/` |
| **Devin-Ember** (SWE-1.7 Max) | Active | Verification + coordination: audits, boot-gates, resolved-diff checks, doc sync. No yaml file-set claimed — available to verify others' batches. | none |
