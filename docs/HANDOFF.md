...
| **Devin-Aurora** (was Devin-Aether, this agent, SWE-1.7 Max) | Active | D2k Phase 0/1/2 coordinator; Atreides pack complete (`f07d8d35e`) and Harkonnen pack complete (`afdaae46c`); now coordinating Corrino (Phase 3) and shared/global pass (Phase 4). | `mods/cameo/ContentPacks/D2k/Atreides/`, `mods/cameo/ContentPacks/D2k/Harkonnen/`, `mods/cameo/bits/d2k/` |
...
| **Corrino** | skeleton only | 2 (MCV, combat tank) | 6+ | 1 (sardaukar bazooka) | 0 | 0 | 0 | **no** (FactionCA active but roster incomplete; needs Death Hand + full tech tree) |
...
| **3 — Corrino** | **Devin-Cyrus** (after WC2 hero blocker and phases 1–2) | `ContentPacks/D2k/Corrino/` (skeleton exists) | Build Corrino as imperial/Sardaukar: 4 infantry (sardaukar variants), 5+ vehicles (combat tank, missile tank, siege tank, uparmored harvester, carryall), 15 buildings (palace with Death Hand), aircraft, weapons, upgrades, sequences, StartingUnits, translations. Add to `Shared/yaml/faction.yaml` if needed. | Same as phase 1. |
| **4 — Shared/global pass** | **Devin-Blaze** + **Devin-Echo** as needed | `ContentPacks/D2k/Shared/yaml/`, `mods/cameo/weapons/d2k.yaml`, `mods/cameo/rules/d2k.yaml` | Add shared templates, fix cross-faction prerequisites, walls/turrets/superweapons/promotions. Remove dead legacy blocks from `mods/cameo/weapons/d2k.yaml` and `mods/cameo/rules/d2k.yaml`. Run `find_empty_warhead.py`, `review_resolve_diff`, `audit_warhead_split`, `extract_stats --check`, full `run_all.py`, and boot-gate. | All audits green; `multi_main_fired_weapons` not inflated. |
