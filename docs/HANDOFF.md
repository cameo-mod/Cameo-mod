...
```
W24  one damage warhead per weapon          884 fired weapons still carry 2+
 └─> W23  retrofit the legacy templates      1126 direct inheritors left; 1248 fired
...
|---|---|---|
| **W24** | collapse each fired weapon to ONE damage warhead (DESIGN §11b) | `multi_main_fired_weapons` in `doc_claims.yaml` goes DOWN from 884 |
| **W23** | retrofit the legacy templates onto `^Warhead_*` families | `unconverted_template_inheritors` goes DOWN from 1126; `warhead_family_reach` goes UP from 1248 |
