...
namespace OpenRA.Mods.CA.Widgets
{
	public class ProductionTabCA
	{
		...
		// Stage B (DecoupledRendering): per-frame render/UI reads (tab visibility, done-highlight, group alert,
	...
	public class ProductionTabsCAWidget : Widget
	{
		...
		// Stage B (DecoupledRendering): world.ActorAdded/ActorRemoved fire on the SIM thread, so this must not
		...
		{
			// Stage B (DecoupledRendering): the queue-list + cached-render refresh MUST run every logic tick
		...
		void RefreshState()
		{
			// Stage B (DecoupledRendering): refresh queue/tab state only when the sim thread is not mid-tick, so
		...
		public override bool HandleMouseInput(MouseInput mi)
		{
			...
			if (offsetloc.X > 0 && offsetloc.X < contentWidth)
			{
				...
				if (Groups[queueGroup].Tabs.Count >= tabIndex)
				{
					...
					// Stage B: switching the queue refreshes the palette icons from live queue state - wait out
		...
		public override bool HandleKeyPress(KeyInput e)
		{
			...
			if (PreviousProductionTabKey.IsActivatedBy(e) || NextProductionTabKey.IsActivatedBy(e))
			{
				...
				// Stage B: SelectNextTab switches the queue, refreshing palette icons from live queue state.
				Game.EnterWorldReadLock();
				try
				{
