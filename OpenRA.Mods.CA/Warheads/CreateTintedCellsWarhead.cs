...
namespace OpenRA.Mods.CA.Warheads
{
	public class CreateTintedCellsWarhead : DamageWarhead, IRulesetLoaded<WeaponInfo>
	{
		...
		[Desc("If true, only paint cells within a cone using the projectile's facing from ImpactOrientation.")]
		public readonly bool UseConeFilter = false;

		[Desc("Half-angle of the cone in degrees. Only used when UseConeFilter is true.")]
		public readonly int ConeAngle = 60;

		[Desc("Minimum distance from source to start painting cells. Filters out cells behind the unit.")]
		public readonly WDist ConeMinRange = WDist.Zero;
		...
		protected override void DoImpact(WPos pos, Actor firedBy, WarheadArgs args)
		{
			...
			// Build cone filter from ImpactOrientation yaw (projectile facing)
			WPos coneSource = pos;
			WVec coneForward = WVec.Zero;
			var halfConeAngleRadians = 0.0;
			var useCone = UseConeFilter && args.Source.HasValue;

			if (useCone)
			{
				coneSource = new WPos(args.Source.Value.X, args.Source.Value.Y, 0);

				// Use ImpactOrientation yaw — this is the projectile's travel direction
				var yaw = args.ImpactOrientation.Yaw;
				coneForward = new WVec(0, -1024, 0).Rotate(WRot.FromYaw(yaw));

				halfConeAngleRadians = ConeAngle * Math.PI / 360.0;
			}
			...
			for (var i = 0; i < Range.Length; i++)
			{
				...
				foreach (var cell in affectedCells)
				{
					if (useCone)
					{
						var cellCenter = new WPos(world.Map.CenterOfCell(cell).X, world.Map.CenterOfCell(cell).Y, 0);
						var toCell = cellCenter - coneSource;

						// Skip cells behind the source or within min range
						if (toCell.Length < ConeMinRange.Length)
							continue;

						if (toCell.Length == 0 || coneForward.Length == 0)
							continue;

						// Dot product to check angle
						var dot = (long)coneForward.X * toCell.X + (long)coneForward.Y * toCell.Y;
						var lenProduct = (long)coneForward.Length * toCell.Length;

						if (lenProduct == 0)
							continue;

						var cosAngle = (double)dot / lenProduct;
