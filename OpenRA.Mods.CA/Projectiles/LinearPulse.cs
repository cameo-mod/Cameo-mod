...
namespace OpenRA.Mods.CA.Projectiles
{
	...
	public class LinearPulseInfo : IProjectileInfo
	{
		...
		[Desc("Name of the TintedCellsLayer to paint when using Cone impact type. Leave empty to disable.")]
		public readonly string ConeTintLayer = null;

		[Desc("Level of tint to apply per cell. Used with ConeTintLayer.")]
		public readonly int ConeTintLevel = 100;

		[Desc("Maximum tint level a cell can reach. Used with ConeTintLayer.")]
		public readonly int ConeTintMaxLevel = 500;

		[Desc("Falloff of tint level based on distance from source. Used with ConeTintLayer.")]
		public readonly int[] ConeTintFalloff = { 100, 75, 50, 25, 0 };
	...
	public class LinearPulse : IProjectile, ISync
	{
		...
		public void Tick(World world)
		{
			...
			// Paint cone tint once when travel is complete
			if (travelComplete && info.ImpactType == LinearPulseImpactType.Cone)
				PaintConeTint();
		...
		void Explode(WPos impactPos)
		{
			...
			switch (info.ImpactType)
			{
				case LinearPulseImpactType.StandardImpact:
					...
					{
						ImpactOrientation = new WRot(WAngle.Zero, WAngle.Zero, facing),
					});
					break;
		...
		void PaintConeTint()
		{
			...
			foreach (var cell in cells)
			{
				...
				var groundCellCenter = new WPos(cellCenter.X, cellCenter.Y, 0);
				var toCell = groundCellCenter - groundSource;
		...
		int GetConeTintFalloff(int distFromSource)
		{
			var falloff = info.ConeTintFalloff;
			if (falloff.Length == 0)
				return 100;

			// Distribute falloff steps evenly across range
			var step = range / Math.Max(falloff.Length - 1, 1);
			if (step == 0)
				return falloff[0];
			...
			{
				var outer = i * step;
				if (distFromSource <= outer)
				{
					var inner = (i - 1) * step;
					return int2.Lerp(falloff[i - 1], falloff[i], distFromSource - inner, outer - inner);
				}
			}
