...
def main():
    ...
    for old, (fam, lvl) in EMAP.items():
        ...
        if not b:
            continue
        fx = extract_fx(b)
        # the firing SOUND (top-level Report / StartReport) is an audio effect — carry it
        # in the effect layer so the split preserves it (and `-Report:` still has a target).
        report = [l for l in b if re.match(r"^\t(Report|StartReport|StartBurstReport):", l)]
        if not fx and not report:
            continue
        name = f"Effect{fam}_{lvl}"
        emitted[name] = "\n".join([f"^{name}:"] + report + fx)
        order.append(name)
