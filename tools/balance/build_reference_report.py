...
def emit(body, members, crows, assignment, attached, chassis_only, dist, cdist, counts, klass, led_arms, hero_context=None):
    for kind, title in SECTIONS:
        ...
        for a in group:
            ...
            # ⭐ LAW 3 BEFORE THE FALLBACK. A multi-gun actor gets its combined total where the
            # sum is proven honest; everything else keeps the WITHHELD marker it had.
            ctype_a = c.get("type")
            combined = (combined_damage_cell(a, c, rows, selected_dist, selected_cdist, ctype_a)
                        if c.get("weapon_model_eligible") is False else None)
            ...
            shared_range = (combined_range_cell(a, tgt["w_range"])
                            if c.get("weapon_model_eligible") is False else None)
