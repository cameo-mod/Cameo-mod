#!/usr/bin/env python3
"""sim_attack_tesla.py — what an `AttackTesla` actor's attack period ACTUALLY is.
...
⭐ This reproduces ALL THREE maintainer rulings exactly — 131, 95 and **220** — including
...
⚠ THE ONE FREE ASSUMPTION: how fast the actor reacquires after the activity ends. This
models it as immediate (`--reacquire`, default 0), because that is what reproduces the
ruled 220. A slower reacquisition lengthens the Rail Tower's period and leaves the two
coils untouched, since they never exit in the first place.
...
CLAIMED = {
    ...
    "asianalliance_railtower": {"maintainer 2026-09-14": 220, "#385": 160,
...
def formula(max_charges, trait_reload, initial_charge, charge_delay, weapon_reload) -> int:
    """The closed form. The simulation below exists to check it, not to be replaced by it."""
...
def main() -> int:
    ...
    for name, spec in ACTORS.items():
        ...
        print(f"{name:<26} {mode:<9} {str(gap):>5} {str(period):>10} {closed:>8}"
    ...
    if args.check:
        for name, spec in ACTORS.items():
            ...
            ruled = CLAIMED[name]["maintainer 2026-09-14"]
            if period != ruled:
