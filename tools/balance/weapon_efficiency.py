...
damage, shield chips, and the folded ``PercentageScale`` hit because all three fall to
zero with the main Damage. A standalone percentage warhead does not, so the honest model
is AFFINE rather than a single multiplier (E4, corrected 2026-08-25):
...
standalone absolute contribution and the folded hit's tiny basis-point rounding residual,
...
rounding for that snapped value.
...
def analyse(resolved, damage_total: float | None = None):
    ...
    # applications are absolute. Folded basis-point quantisation is reported as a
    # separate current-shot residual so it cannot masquerade as a permanent floor.
