...
This module mirrors the arithmetic in ``AreaDamageWarhead.cs`` and
``AreaDamagePercentageWarhead.cs``.  It deliberately knows nothing about target
density or projectile reliability; callers layer those concerns on top of the
same runtime applications returned here.
"""
...
def folded_units(damage: int, scale: int) -> tuple[float, int]:
    ...
    using integer division.  The continuous value is kept separately because it
    is the scalable coefficient; the small difference to the rounded value is a
    current-shot quantisation residual, not a permanent damage floor.
    """
    continuous = damage * scale / FOLDED_SCALE_DENOMINATOR
    rounded = _truncate_div(
        damage * scale + FOLDED_ROUNDING_BIAS, FOLDED_SCALE_DENOMINATOR)
    rounded = min(max(rounded, INT32_MIN), INT32_MAX)
    return continuous, rounded


def runtime_percentage_hp(reference_hp: float, units: int, denominator: int) -> int:
    """Neutral-armor HP damage with the same truncations and clamp as C#."""
    hp = int(reference_hp)
    after_units = _truncate_div(hp * units, 100)
    result = _truncate_div(after_units * 100, denominator)
    return min(max(result, INT32_MIN), INT32_MAX)
