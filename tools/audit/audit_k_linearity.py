...
Three checks:
  ...
  **L2 — the measurement identity holds**, including the folded basis-point rounding residual.
...
def main() -> int:
    ...
    print("`k == k_flat + (pct_absolute + folded_rounding) / damage_total`. The "
          "standalone term is a floor; folded rounding is only the current quantisation residual.")
    print()
    broken = []
    undefined = []
    ...
    # ---- L4 -----------------------------------------------------------------
    print("## L4 — folded basis-point rounding residual")
    print()
    rounded = sorted(
        ((name, res["folded_rounding_context"])
         for name, _node, res in rows if abs(res["folded_rounding_context"]) >= 1e-9),
        key=lambda row: -abs(row[1]))
    print(f"{len(rounded)} weapon(s) have a non-zero current folded rounding residual.")
