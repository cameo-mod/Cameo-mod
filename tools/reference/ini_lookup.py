...
def main() -> int:
    ...
    ap.add_argument("source", choices=[s[0] for s in wm.INI_SOURCES])
    ap.add_argument("warheads", nargs="+")
    ap.add_argument("--max-weapons", type=int, default=4)
    args = ap.parse_args()

    src = next(s for s in wm.INI_SOURCES if s[0] == args.source)
    ini = wm.parse_ini(src[2])
    ...
    for name in args.warheads:
        ...
        if key is None:
            print(f"\n##### {name}: NOT a section of {src[2].name}")
            continue
