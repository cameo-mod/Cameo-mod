...
class WeaponStructureInventoryTests(unittest.TestCase):
    ...
    def test_current_corrected_baseline(self):
        ...
        self.assertEqual(2725, self.data["counts"]["main_warhead_instances_all_concrete"])
        self.assertEqual(696, self.data["counts"]["excess_main_warhead_instances_all_concrete"])
        self.assertEqual(2299, self.data["counts"]["main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(551, self.data["counts"]["excess_main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(126, self.data["counts"]["reviewed_stacked_main_all_concrete"])
        self.assertEqual(98, self.data["counts"]["reviewed_stacked_main_direct_actor_armament"])
        self.assertEqual(28, self.data["counts"]["reviewed_stacked_main_indirect_weapon_graph"])
        self.assertEqual(126, self.data["counts"]["reviewed_stacked_main_transitive_weapon_graph"])
        self.assertEqual(261, self.data["counts"]["unreviewed_stacked_main_all_concrete"])
        self.assertEqual(161, self.data["counts"]["unreviewed_stacked_main_transitive_weapon_graph"])
        self.assertEqual(254, self.data["counts"]["reviewed_excess_main_warhead_instances_all_concrete"])
        self.assertEqual(254, self.data["counts"]["reviewed_excess_main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(442, self.data["counts"]["unreviewed_excess_main_warhead_instances_all_concrete"])
        self.assertEqual(297, self.data["counts"]["unreviewed_excess_main_warhead_instances_transitive_weapon_graph"])

    def test_raw_reachable_ratchets_match_the_checkpoint(self):
        self.assertEqual(287, RAW_REACHABLE_BASELINE)
        self.assertEqual(551, RAW_REACHABLE_EXCESS_BASELINE)
