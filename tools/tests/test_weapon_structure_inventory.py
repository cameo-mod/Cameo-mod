...
class WeaponStructureInventoryTests(unittest.TestCase):
    ...
    def test_current_corrected_baseline(self):
        ...
        self.assertEqual(387, self.data["counts"]["stacked_main_all_concrete"])
        self.assertEqual(232, self.data["counts"]["stacked_main_direct_actor_armament"])
        self.assertEqual(55, self.data["counts"]["stacked_main_indirect_weapon_graph"])
        self.assertEqual(287, self.data["counts"]["stacked_main_transitive_weapon_graph"])
        self.assertEqual(100, self.data["counts"]["stacked_main_unreached"])
        self.assertEqual(2724, self.data["counts"]["main_warhead_instances_all_concrete"])
        self.assertEqual(695, self.data["counts"]["excess_main_warhead_instances_all_concrete"])
        self.assertEqual(2298, self.data["counts"]["main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(550, self.data["counts"]["excess_main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(212, self.data["counts"]["reviewed_stacked_main_all_concrete"])
        self.assertEqual(165, self.data["counts"]["reviewed_stacked_main_direct_actor_armament"])
        self.assertEqual(47, self.data["counts"]["reviewed_stacked_main_indirect_weapon_graph"])
        self.assertEqual(212, self.data["counts"]["reviewed_stacked_main_transitive_weapon_graph"])
        self.assertEqual(175, self.data["counts"]["unreviewed_stacked_main_all_concrete"])
        self.assertEqual(75, self.data["counts"]["unreviewed_stacked_main_transitive_weapon_graph"])
        self.assertEqual(413, self.data["counts"]["reviewed_excess_main_warhead_instances_all_concrete"])
        self.assertEqual(413, self.data["counts"]["reviewed_excess_main_warhead_instances_transitive_weapon_graph"])
        self.assertEqual(282, self.data["counts"]["unreviewed_excess_main_warhead_instances_all_concrete"])
        self.assertEqual(137, self.data["counts"]["unreviewed_excess_main_warhead_instances_transitive_weapon_graph"])

    def test_raw_reachable_ratchets_match_the_checkpoint(self):
        self.assertEqual(287, RAW_REACHABLE_BASELINE)
        self.assertEqual(550, RAW_REACHABLE_EXCESS_BASELINE)
