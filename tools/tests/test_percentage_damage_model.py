...
class PercentageArithmeticTest(unittest.TestCase):
    def test_csharp_percentage_paths_keep_wide_intermediates(self):
        area = (ROOT / "OpenRA.Mods.Cameo" / "Warheads" /
                "AreaDamageWarhead.cs").read_text(encoding="utf-8")
        standalone = (ROOT / "OpenRA.Mods.Cameo" / "Warheads" /
                      "AreaDamagePercentageWarhead.cs").read_text(encoding="utf-8")
        self.assertIn("damage * (long)scale", area)
        self.assertIn("decimal.Truncate(result * 100m / denominator)", area)
        self.assertIn("ApplyPercentageDamage(healthInfo.HP", area)
        self.assertIn("ApplyPercentageDamage(healthInfo.HP", standalone)
    ...
    def test_folded_arithmetic_handles_live_values_beyond_int32_product(self):
    ...
    def test_percentage_runtime_mirror_clamps_final_int32_overflow(self):
        self.assertEqual(
            pd.runtime_percentage_hp(pd.INT32_MAX, 200, 100), pd.INT32_MAX)
        self.assertEqual(
            pd.folded_units(pd.INT32_MAX, pd.INT32_MAX)[1], pd.INT32_MAX)
    ...
    def test_standalone_area_percentage_rejects_invalid_runtime_shape(self):
