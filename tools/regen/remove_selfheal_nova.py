#!/usr/bin/env python3
"""Remove ChangesHealth@SelfHealing Step-only nodes from Nova's ContentPacks.

Nova's packs: RedAlert/*, RedAlert2/*, RedAlert2Mod/* (except Consortium —
already done by Ember).
...
Usage:
  python tools/regen/remove_selfheal_nova.py

Then commit and push to devin/regen/conversion:
  git add mods/cameo/ContentPacks/RedAlert*/**/yaml/*.yaml
  git commit -m "regen(ra/ra2/ra2mod): remove N ChangesHealth@SelfHealing Step overrides"
  git push origin devin/nova/w24-lane2:devin/regen/conversion
"""
import pathlib
import re

ROOT = pathlib.Path("mods/cameo/ContentPacks")
MY_PACKS = ["RedAlert", "RedAlert2", "RedAlert2Mod"]
# Consortium already done by Ember
SKIP_SUBPACKS = {"RedAlert2Mod/Consortium"}
